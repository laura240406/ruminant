...
class IsoModule(module.RuminantModule):
    ...
    def parse_mdat(self, atoms):
        if self.get_all(atoms, "ftyp")[0]["data"]["major-brand"] in ("avif", "heic", "mif1", "vvic"):
    ...
    def process_heif_mdat(self, atoms):
        ...
        codec = None
        for atom in ipco:
            if atom["type"] in ("av1C", "hvcC", "avcC", "vvcC"):
                codec = atom["type"]
                break
    ...
    def process_heif_picture(self, codec, picture):
        ...
        match codec:
            ...
            case "vvcC":
                picture["nals"] = []
                while picture["buf"].available():
                    picture["nals"].append(FFMpreg.read_h266_nalu(picture["buf"]))
            ...
            match stream["codec"]:
                ...
                case "V_PRORES":
                    ...
                    for index in ranges:
                        ...
                        stream["samples"][index] = FFMpreg.read_prores_packet(self.buf)
        ...
        while buf.unit > 0:
            ...
            match desc["tag"]:
                ...
                case 0x6a:
                    ...
                    desc["data"]["bsid-flag"] = buf.rb(1)
                    desc["data"]["mainid-flag"] = buf.rb(1)
                    desc["data"]["asvc-flag"] = buf.rb(1)
                    desc["data"]["reserved"] = buf.rb(4)

                    if desc["data"]["component-type-flag"]:
                        desc["data"]["component-type"] = {"audio-service-mode": buf.rb(5), "channel-configuration": buf.rb(3)}

                    if desc["data"]["bsid-flag"]:
                        desc["data"]["bsid"] = buf.ru8()

                    if desc["data"]["mainid-flag"]:
                        desc["data"]["mainid"] = buf.ru8()

                    if desc["data"]["asvc-flag"]:
                        desc["data"]["asvc"] = buf.ru8()

                    if buf.unit > 0:
                        desc["data"]["rest"] = buf.rh(buf.unit)
                case 0x59:
                    desc["type"] = "Subtiltling Descriptor"
                    desc["data"]["language"] = buf.rs(3)
                    desc["data"]["subtitling-type"] = {
                        "type": utils.unraw(
                            buf.rb(4),
                            1,
                            {0x00: "Teletext", 0x01: "Normal DVB", 0x02: "Hard of Hearing", 0x03: "Sign Language"},
                            True,
                        ),
                        "resolution": utils.unraw(
                            buf.rb(4),
                            1,
                            {0x00: "None", 0x01: "4:3", 0x02: "16:9", 0x03: "2.21:1", 0x04: "HD", 0x05: "3D"},
                            True,
                        ),
                    }
...
class DiracModule(module.RuminantModule):
    ...
    def chew(self) -> ruminant_types.JSON:
        ...
        while not should_break:
            packet = FFMpreg.read_dirac_packet(self.buf)
            should_break = packet["next-offset"] == 13
...
class JvtNalH264Module(module.RuminantModule):
    ...
    def chew(self) -> ruminant_types.JSON:
        ...
        meta["nals"] = []
        for i in ranges:
            self.buf.seek(starts[i][0])
            self.buf.pasunit(starts[i][1])

            meta["nals"].append(FFMpreg.read_h264_nalu(self.buf, slim=True))

            self.buf.popunit()

        self.buf.seek(starts[-1][0])
        self.buf.pasunit(starts[-1][1])
